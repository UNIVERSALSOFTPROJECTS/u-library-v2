...
{#if open}
    <div class="{subModalOpened?'sub':''} modal {modalOpened || subModalOpened}" on:touchstart={lockTouchZoom} on:touchmove={lockTouchZoom} on:click={() => (open = false)} use:watchResize={resizeHeightModal}>
