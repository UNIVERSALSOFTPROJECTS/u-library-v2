<script>
  ...
  const openPinnacle = async () => {
    let url =
      userState == "loggedIn"
        ? ut.getGameURL(GAMEAPI_URL, games.pinnacle, options.gameToken)
        : guestURLBBQ;
    url +=
      active_view == "sportbooklive" ? "&sport_view=live" : "&sport_view=sport";
    url += `&lang=${lang}&r=url`;
  ...
  const openBBQ = async () => {
    let url =
      userState == "loggedIn"
        ? ut.getGameURL(GAMEAPI_URL, games.bbq, options.gameToken)
        : guestURLBBQ;
  ...
  const openDigtain = async () => {
    let url =
      userState == "loggedIn"
        ? ut.getGameURL(GAMEAPI_URL, games.digtain, options.gameToken)
        : guestURLdigtain;
    url +=
      active_view == "sportbooklive"
        ? "&currentgame=live"
        : "&currentgame=PreMatch";
    url += `&lang=${lang}`;
    if (options.eventInfo) {
      const eventInfo = JSON.stringify(
        mode == "wb" ? options.eventInfo : options.eventInfo.Id
      );
  ...
  const openWintech = async () => {
    let url =
      userState == "loggedIn"
        ? ut.getGameURL(GAMEAPI_URL, games.wintech, options.gameToken)
        : baseUrlWintech;
  ...
  const openNovusbet = async () => {
    let url =
      userState == "loggedIn"
        ? ut.getGameURL(GAMEAPI_URL, games.novusbet, options.gameToken)
        : baseUrlNovusbet;
    url += active_view == "sportbooklive" ? "&page=live" : "&page=sport";
    if (userState == "loggedIn") {
      const { data } = await backend.game.getURLNovus(url);
