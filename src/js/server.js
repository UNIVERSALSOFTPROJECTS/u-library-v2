...
const ServerConnection = (() => {
    ...
    const users = {
        ...
        verifyRollover: async (token) => {
            let url = conf.API_KS + `/verifyRollover?token=${token}`;
    ...
    const u_user = {
        ...
        generateSignatureToOrderPayIn:async (payload,usertoken_nc) => {
            return await axios.post("http://localhost:5000/wallet-service" + "/webhooks/payment/nxpay/signature", payload,{
                                                headers: {
                                                "Content-Type": "application/json;charset=UTF-8",
